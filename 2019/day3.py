...
def get_coord(instructions):
    setAxisX = {'R': 1, 'U': 0, 'L': -1,'D': 0}
    setAxisY = {'R': 0, 'U': 1, 'L': 0,'D': -1}
    coordinates = {}
    pathLen = 0
    x, y = 0, 0    
    for move in instructions:
        instruction = move[0]
        moveSize = int(move[1:])
        for _ in range(moveSize):
            x += setAxisX[instruction]
            y += setAxisY[instruction]
            pathLen += 1
            if not (x,y) in coordinates.keys():
                coordinates[(x,y)] = pathLen
    return coordinates

def get_intersections(coordinates1, coordinates2):
    return coordinates1.keys() & coordinates2.keys()

def get_min(intersections):
    distances = []
    for coord in intersections:
        distances.append(abs(coord[0])+abs(coord[1]))
    return min(distances)

def get_minPath(intersections, inArr1, inArr2):
    lowest = abs(inArr1[list(intersections)[0]]) + \
             abs(inArr2[list(intersections)[0]])
    for crossing in intersections:
        if (abs(inArr1[crossing]) + \
            abs(inArr2[crossing])) < lowest:
            lowest = (abs(inArr1[crossing]) + \
                      abs(inArr2[crossing]))
        else:
            lowest = lowest
    return lowest
    
coord1 = get_coord(rawIn1.split(','))
coord2 = get_coord(rawIn2.split(','))
inters = get_intersections(coord1, coord2)
print(get_min(inters))
print(get_minPath(inters, coord1, coord2))
