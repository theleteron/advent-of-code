STRING_INPUT="""[1518-05-11 00:22] falls asleep
...
def parseRow(inputRow):
    rowSplit = inputRow.strip('[').strip(']').split(maxsplit=2)
    date = rowSplit[0].split('-')
    year = int(date[0])
    month = int(date[1])
    day = int(date[2])
    hours = rowSplit[1].split(':')
    hour = int(hours[0])
    minute = int(hours[1])
    status = rowSplit[2]
    return year, month, day, hour, minute, status

def day4_part1(inputString):
